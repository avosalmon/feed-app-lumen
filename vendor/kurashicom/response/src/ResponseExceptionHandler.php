...
class ResponseExceptionHandler {
	...
	{
		if ($exception instanceof NotFoundHttpException) return $this->response->error('', '404');

		if ($exception instanceof ModelNotFoundException) return $this->response->error('', '404');

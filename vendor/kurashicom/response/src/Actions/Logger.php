...
class Logger {
	...
	{
		if ($this->env->get('APP_ENV') == 'testing') return $this->logToConsole($message);
	...
	public function logToFile($message = '', $log = '400')
	{
		...
		if($message === '') $message = $this->request->server('REQUEST_METHOD') . ' ' . $this->request->server('REQUEST_URI') . ' | ' . $this->request->server('HTTP_X_REAL_IP') . ' ('. $this->request->server('REMOTE_ADDR') . ') | ';
